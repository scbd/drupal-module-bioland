 ...
 * (currently bioland_update_9082()) must re-apply the canonical v2 config.
 *
 * @group bioland
 * @coversNothing
 */
class SearchApiConvergenceHookTest extends TestCase {
  ...
  public function testConvergenceHookAppliesCanonicalV2Config(): void {
    ...
    // The new highest-numbered hook (bioland_update_9082(), added for BL-1015
  ...
  public function testConvergenceHookRunsLast(): void {
    ...
    $this->assertSame(
      9082,
      max($numbers),
      'The highest-numbered update hook must converge every site last. bioland_update_9082() now holds that role (it seeds the Google Analytics switch as disabled and re-applies the canonical v2 config after the 9071-9081 corrective hooks); if you add a higher-numbered hook it must itself converge on the v2 config and this test must be updated to point at it.'
    );
  }
