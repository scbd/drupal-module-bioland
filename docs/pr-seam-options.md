...
| C5's JS calls `once()`, which AGENTS.md forbids outright | codex #4 | **Held, and escalated as an open question.** The breach exists at HEAD today (`js/bioland-component-menu-form-1-1-6.js:63`); it is a pre-existing code defect, not a seam defect. Seaming must not silently launder it — fix it in C5 or file it as its own ticket before C5 is opened |
...
3. `js/bioland-component-menu-form-1-1-6.js:63` calls `once()`, which AGENTS.md forbids ("NEVER use `.once()` or `once()`"). Fix it inside C5 with the documented `dataset` guard, or file a separate ticket first? (Recommend: fix inside C5 — the file is new in that seam, so there is no history to disturb.)
4. Whether staging processes require the hook-bearing X1 to deploy in the same release as C7/C8 (the seeded `component_menu_*` config) — if staging deploys per-PR, X1's hook is the only update-path event and must be release-noted.
